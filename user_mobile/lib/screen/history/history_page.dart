...
class _HistoryPageState extends State<HistoryPage> {
  Future<void> _showPhotoPreview(String url, {required String title}) async {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AppProvider>().fetchAttendanceByMonth(
        _selectedMonth,
        _selectedYear,
      );
    });
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<int>(
          ...
          items: List.generate(
            12,
            (index) =>
                DropdownMenuItem(value: index + 1, child: Text(months[index])),
          ),
        ),
      ),
    );
      ...
      child: Column(
        ...
        children: [
          Icon(
            ...
            color: zenSlate.withValues(alpha: 0.2),
            size: 64,
          ),
  ...
  Widget _buildAttendanceCard(BuildContext context, AttendanceRecord r) {
    ...
    if (isLeave) {
      statusColor = zenIndigo;
      statusLabel = 'AUTHORIZED';
    }

    String checkInTime = r.checkIn?.time != null
        ? DateFormat('HH:mm').format(r.checkIn!.time!)
        : '--:--';
    String checkOutTime = r.checkOut?.time != null
        ? DateFormat('HH:mm').format(r.checkOut!.time!)
        : '--:--';

    final dispute = context
        .watch<AppProvider>()
        .myDisputeRequests
        .where((d) => d.relatedAttendanceId == r.id)
        .firstOrNull;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Builder(
            builder: (ctx) {
              final checkInPhoto = r.checkIn?.photoUrl;
              final checkOutPhoto = r.checkOut?.photoUrl;
              final hasAnyPhoto =
                  (checkInPhoto != null && checkInPhoto.isNotEmpty) ||
                  (checkOutPhoto != null && checkOutPhoto.isNotEmpty);
                  ...
                  child: Row(
                    children: [
                      Icon(
                        ...
                        size: 16,
                        color: zenSlate.withValues(alpha: 0.7),
                      ),
                      const SizedBox(width: 8),
                      Text(
                        'Foto tidak tersedia',
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: zenRose.withValues(alpha: 0.3),
                          blurRadius: 15,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (dispute.adminResponse != null &&
                        dispute.adminResponse!.isNotEmpty) ...[
                      const SizedBox(height: 6),
                      Text(
                        'Tap untuk buka percakapan dengan admin',
                        style: GoogleFonts.plusJakartaSans(
                          color: zenNavy.withValues(alpha: 0.7),
                          fontSize: 10,
  ...
  Widget _photoTile({
    ...
    return AspectRatio(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(18),
        onTap: hasUrl ? () => _showPhotoPreview(url!, title: title) : null,
        child: Ink(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(18),
            color: hasUrl ? accentColor.withValues(alpha: 0.08) : Colors.white,
            border: Border.all(
              color: hasUrl
                  ? accentColor.withValues(alpha: 0.25)
                  : zenNavy.withValues(alpha: 0.06),
              width: 1,
            ),
          ),
          child: Stack(
            children: [
              ...
              if (!hasUrl)
                _tilePlaceholder(accentColor: accentColor, title: title),
  ...
  Widget _tilePlaceholder({required Color accentColor, required String title}) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          ...
          Text(
            'NO PHOTO',
            style: GoogleFonts.outfit(
              color: zenSlate.withValues(alpha: 0.8),
              fontSize: 9,
              fontWeight: FontWeight.w900,
              letterSpacing: 1,
            ),
